...
export default function SiteEditor() {
  ...
  // Style tab (separate from settings drawer — local until "Save Style" hits settings)
  ...
  // Fetch blocks when activePage changes
  useEffect(() => {
  ...
  async function handlePublish() {
    try {
      ...
      toast(err instanceof Error ? err.message : "Failed to publish", true);
    }
  }
  ...
  async function handleApplyTemplate(templateId: string) {
    try {
      ...
      // Refresh pages/blocks since they were replaced
      setPages([]);
      setActivePage(null);
      setBlocks([]);
  ...
  return (
    <div className="mgr-outer" style={{ height: "100vh", overflow: "hidden" }}>
        ...
        <div className="hub-shell">
          <div className="hub-left">
            <div className="hub-brand">
              ...
              <p className="hub-subtitle">Site Manager</p>
            </div>
            <nav className="hub-tiles">
              {([
                ["website",    "Website",    "Pages, blocks & site style"],
                ["photos",     "Photos",     "Upload & manage your library"],
                ["guestlist",  "Guest List", "Add, import & track RSVPs"],
                ["templates",  "Templates",  "Save snapshots & publish"],
                ["site-setup", "Site Setup", "Domain, type & settings"],
                ["analytics",  "Analytics",  "Page views & RSVP stats"],
              ] as const).map(([key, name, desc]) => (
                <button key={key} className="hub-tile" onClick={() => setSection(key as Section)}>
                  <span className="hub-tile-name">{name}</span>
                  <span className="hub-tile-desc">{desc}</span>
                </button>
              ))}
            </nav>
            <a href={siteUrl} target="_blank" rel="noreferrer" className="hub-view-site">
              View live site ↗
            </a>
          </div>
      ...
      {section === "website" && (
        <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
          <div className="section-topbar">
            <button className="back-btn" onClick={() => setSection("hub")}>
              ← Dashboard
            </button>
            ...
            <button className="btn-primary-sm" onClick={handlePublish}>Publish</button>
            <div className="section-topbar-divider" />
      ...
      {section === "photos" && (
        <div style={{ display: "flex", flexDirection: "column", height: "100vh" }}>
          <div className="section-topbar">
            <button className="back-btn" onClick={() => setSection("hub")}>← Dashboard</button>
            <div className="section-topbar-divider" />
