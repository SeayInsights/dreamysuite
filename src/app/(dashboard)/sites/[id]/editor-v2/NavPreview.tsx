...
function LangToggle({
  ...
  const label = displayLang
    ? (LANG_NAMES[mainLang] ?? mainLang)
    : (LANG_NAMES[langs[0]] ?? langs[0]);
...
export function NavPreview() {
  ...
  const NavStyleEffect = useMemo(
    () =>
      settings.effectNavStyle
        ? getEffectComponent(settings.effectNavStyle)
        : null,
    [settings.effectNavStyle],
  );

  const additionalLangs = useMemo(
    () => parseSiteLanguages(settings.siteLanguages ?? null),
    [settings.siteLanguages],
  );
  ...
  if (NavStyleEffect) {
    ...
    const initials = eventName
      ...
      .slice(0, 3);
    const accent =
      settings.navHighlightColor ?? themeTokens.colors.primary ?? "#B8921A";
    ...
    const headingFont =
      themeTokens.typography.headingFont || "system-ui, sans-serif";
