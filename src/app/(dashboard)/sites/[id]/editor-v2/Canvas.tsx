...
export function Canvas({ siteId }: Props) {
  ...
  const rawBlocks = useEditorStore(useShallow((s) => s.blocks));
  ...
  const blocks = useMemo(() => {
    ...
    return [...withEffectiveConfig].sort((a, b) => {
      const indexA = rawBlocks.indexOf(rawBlocks.find((rb) => rb.id === a.id)!);
      const indexB = rawBlocks.indexOf(rawBlocks.find((rb) => rb.id === b.id)!);
    ...
    async function loadPages() {
      try {
        ...
        const { pages } = (await pagesRes.json()) as {
          pages: import("@/app/stores/slices/editorShell").EditorPage[];
        };
      ...
      try {
        ...
        if (!cancelled) {
          ...
          const {
            ...
            fixed,
            unchanged,
          } = migrateOutOfBoundsElements(consolidated as Block[]);
          if (fixed > 0) {
          ...
          for (let i = 0; i < consolidated.length; i++) {
            ...
            if (
              before.config.top !== after.config.top ||
