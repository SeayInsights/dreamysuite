...
export function GuestsPanel({ onBack }: { onBack: () => void }) {
	...
	const badge = (s: Guest["rsvpStatus"]) => (
		<span
			...
			style={{
				background: s === "yes" ? "#dcfce7" : s === "no" ? "#fef2f2" : "#f5f5f4",
	...
	function cell(g: Guest, field: string, display: ReactNode, tdClass = "px-2 py-1.5") {
		...
		const startEdit = () => {
			...
			setEditVal(String((g as unknown as Record<string, unknown>)[field] ?? ""));
		};
			...
			return (
				<td key={field} className={tdClass}>
					<select
						...
						value={editVal}
						onChange={(e) => { patchGuest(g.id, field, e.target.value); }}
		...
		return (
			<td key={field} className={`${tdClass} cursor-pointer`} onClick={startEdit}>
				{display}
			</td>
		);
	}
	...
	function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
		...
		reader.onload = (ev) => {
			...
			headers.forEach((h) => {
				const match = GUEST_FIELDS.find((f) => f.toLowerCase() === h.toLowerCase());
				mapping[h] = match ?? "";
			});
	...
	async function submitImport() {
		...
		try {
			const res = await fetch(`/api/sites/${siteId}/guests/import`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({ rows: importModal.rows, mapping: importModal.mapping }),
			});
			if (!res.ok) return;
	...
	return createPortal(
		<div className="fixed inset-0 z-[10000] flex flex-col bg-background text-foreground">
			<div className="flex h-12 shrink-0 items-center gap-3 border-b border-border px-4">
				...
				<button type="button" onClick={() => setShowModal(true)} className="flex items-center gap-1.5 rounded-md bg-foreground px-3 py-1 text-xs font-medium text-background hover:opacity-90">
			...
			<div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-2">
				...
				<select value={filterCat} onChange={(e) => setFilterCat(e.target.value)} className="rounded-md border border-border bg-background px-2 py-1 text-xs outline-none focus:border-ring">
					<option value="">All categories</option>
					{categories.map((c) => <option key={c} value={c}>{c}</option>)}
				</select>
				<select value={filterRsvp} onChange={(e) => setFilterRsvp(e.target.value)} className="rounded-md border border-border bg-background px-2 py-1 text-xs outline-none focus:border-ring">
					<option value="">All RSVPs</option>
					<option value="pending">Pending</option>
					<option value="yes">Attending</option>
					<option value="no">Declined</option>
				</select>
				<select value={filterCeremony} onChange={(e) => setFilterCeremony(e.target.value)} className="rounded-md border border-border bg-background px-2 py-1 text-xs outline-none focus:border-ring">
					<option value="">All events</option>
					<option value="ceremony">Ceremony</option>
					<option value="reception">Reception</option>
					<option value="both">Both</option>
				</select>
				<select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="rounded-md border border-border bg-background px-2 py-1 text-xs outline-none focus:border-ring">
			...
			<div className="flex-1 overflow-auto">
				{loading ? (
					...
					<table className="w-full border-collapse text-xs">
						<thead>
							<tr className="border-b border-border bg-muted/30">
								{COLS.map(([k, en, vi]) => (
									...
									>
										{en}
										{sortCol === k && <span className="ml-0.5 text-[10px]">{sortDir === "asc" ? "▲" : "▼"}</span>}
										{lang === "vi" && <><br /><span className="font-normal opacity-70">{vi}</span></>}
									</th>
								))}
						...
						<tbody>
							{filtered.map((g, i) => (
								<tr key={g.id} className="group border-b border-border/50 hover:bg-accent/20">
									...
									<td
										...
										onClick={() => patchGuest(g.id, "invited", g.invited === 1 ? 0 : 1)}
									>
										{g.invited === 1
									...
									<td
										className="px-2 py-1.5 cursor-pointer"
										onClick={() => patchGuest(g.id, "thankYouSent", g.thankYouSent === 1 ? 0 : 1)}
									>
										{g.thankYouSent === 1
											? <Check className="size-3 text-green-600" />
											: <span className="inline-block size-3 rounded-full border border-muted-foreground/40" />}
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowModal(false)}>
					<form onSubmit={addGuest} onClick={(e) => e.stopPropagation()} className="flex w-80 flex-col gap-3 rounded-lg border border-border bg-background p-4 shadow-xl">
						...
						<div className="flex gap-2">
							{sel("Ceremony/Reception", "ceremonyOrReception", ["ceremony", "reception", "both"])}
							{sel("Invitation Type", "invitationType", ["digital", "printed", "both"])}
						</div>
						...
						<div className="flex justify-end gap-2">
							...
							<button type="submit" disabled={saving || !form.firstName.trim()} className="rounded-md bg-foreground px-3 py-1.5 text-xs font-medium text-background hover:opacity-90 disabled:opacity-50">
					...
					<div onClick={(e) => e.stopPropagation()} className="flex w-72 flex-col gap-3 rounded-lg border border-border bg-background p-4 shadow-xl">
							...
							{categories.map((cat, i) => (
								<li key={cat} className="flex items-center gap-1 rounded-md border border-border px-2 py-1">
									...
									<button
										type="button"
										onClick={() => deleteCat(i)}
										title={guests.some((g) => g.category === cat) ? "In use" : "Delete"}
						...
						<div className="flex gap-2">
							<input
								...
								className={`${IN} flex-1`}
							/>
							<button type="button" onClick={addCat} disabled={!newCat.trim()} className="flex items-center gap-1 rounded-md bg-foreground px-2 py-1.5 text-xs font-medium text-background hover:opacity-90 disabled:opacity-50">
								<Plus className="size-3" />
							</button>
						</div>
						<div className="flex justify-end">
							<button type="button" onClick={() => setShowCatMgr(false)} className="rounded-md border border-border px-3 py-1.5 text-xs hover:bg-accent/30">Done</button>
						</div>
					</div>
				</div>
			)}
											...
											<tbody>
												{importModal.rows.slice(0, 5).map((row, i) => (
													<tr key={i} className="border-t border-border/50">
														{importModal.headers.map((h) => (
															<td key={h} className="max-w-[80px] truncate px-2 py-1">{row[h]}</td>
														))}
													</tr>
												))}
											</tbody>
										...
										{importModal.headers.map((h) => (
											<div key={h} className="flex items-center gap-2">
												...
												<select
													...
													onChange={(e) => setImportModal((p) => p ? { ...p, mapping: { ...p.mapping, [h]: e.target.value } } : p)}
													className="flex-1 rounded border border-border bg-background px-1.5 py-0.5 text-xs outline-none focus:border-ring"
												>
													<option value="">(skip)</option>
													{GUEST_FIELDS.map((f) => <option key={f} value={f}>{f}</option>)}
												</select>
											</div>
										))}
									</div>
