...
const GUESTBOOK = {
  ...
  label: "Guest Book",
  Icon: BookOpen,
  blockType: "guest-book",
} as const;
const TIMELINE = {
  id: "timeline",
  label: "Timeline",
  Icon: Clock,
  blockType: "story-timeline",
} as const;
const INFO_CARD = {
  id: "info-card",
  label: "Info Card",
...
const EVENT_BLOCKS: Record<string, { label: string; blocks: BlockDef[] }> = {
  ...
  anniversary: {
    ...
    blocks: [
      GUESTBOOK,
      { ...TIMELINE, label: "Through the Years" },
      SCHEDULE,
      REGISTRY,
      FAQ,
      FUN_FACTS,
...
export function ElementsTray() {
  ...
  function addBlock(blockType: string) {
    ...
    if (!entry) return;
    // Give the new block a visible, in-front position instead of top:0 (where
    // tray-inserted blocks used to stack invisibly and become impossible to
    // grab). Mirrors InsertPalette's centered-position pattern, plus a small
    // per-insert cascade so consecutive adds don't land on the exact same spot.
    const centered = getCenteredPosition(
      typeof window !== "undefined" ? window.innerHeight : 900,
    );
    const cascade = (blocks.length % 6) * 48;
    const config =
      entry.defaultData.blockOffsetY === undefined
        ? {
            ...entry.defaultData,
            blockOffsetY: centered.blockOffsetY + cascade,
            blockOffsetX: 24 + (blocks.length % 6) * 24,
            zIndex: centered.zIndex,
          }
        : entry.defaultData;
    const newBlock = {
