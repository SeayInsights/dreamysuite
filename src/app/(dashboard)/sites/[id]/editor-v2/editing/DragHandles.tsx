...
export function DragHandles({ containerRef }: Props) {
  ...
  useEffect(() => {
    ...
    const iframeScroll = containerRef.current?.closest(".editor-canvas-scroll");
    iframeScroll?.addEventListener("scroll", schedMeasure);

    return () => {
      ro.disconnect();
      window.removeEventListener("resize", schedMeasure);
      window.removeEventListener("scroll", schedMeasure, true);
      iframeScroll?.removeEventListener("scroll", schedMeasure);
      if (rafRef.current) cancelAnimationFrame(rafRef.current);
    };
