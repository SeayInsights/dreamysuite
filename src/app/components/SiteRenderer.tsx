  ...
  const visible = useMemo(() => {
    ...
    if (breakpoint !== "desktop") {
      return [...filtered].sort((a, b) => {
        const cfgA = (
          typeof a.config === "object" && a.config !== null ? a.config : {}
        ) as Record<string, unknown>;
        const cfgB = (
          typeof b.config === "object" && b.config !== null ? b.config : {}
        ) as Record<string, unknown>;
        const yA =
          typeof cfgA.blockOffsetY === "number" ? cfgA.blockOffsetY : 0;
        const yB =
          typeof cfgB.blockOffsetY === "number" ? cfgB.blockOffsetY : 0;
        return yA - yB;
      });
    }
    return filtered;
  ...
  return (
    <div className="site-renderer relative" style={containerStyle}>
      {translated.map((block) => (
        <MemoBlock key={block.id} block={block} breakpoint={breakpoint} />
      ))}
    </div>
  );
}
