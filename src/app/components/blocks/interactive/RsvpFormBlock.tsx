...
export function RsvpFormBlock({ block }: { block: Block }) {
  ...
  const customQuestions: CustomQuestion[] = Array.isArray(cfg.customQuestions)
    ? (cfg.customQuestions as CustomQuestion[])
    : [];

  // Form submission with useFormSubmit hook
  const endpoint = siteSlug
    ? `/api/public/${siteSlug}/rsvp`
    : `/api/sites/${siteId}/rsvp`;
  ...
  return (
    ...
    >
      <TextEffectWrapper as="h2" className="section-heading">
        {heading}
      </TextEffectWrapper>
