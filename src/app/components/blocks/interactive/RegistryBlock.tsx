...
export function RegistryBlock({ block }: { block: Block }) {
  ...
  const items: RegistryItem[] = Array.isArray(cfg.items)
    ? (cfg.items as RegistryItem[]).filter(
  ...
  const addStoreItem = useCallback(
    (store: string) => {
      ...
      const currentItems = Array.isArray(currentCfg.items)
        ? (currentCfg.items as RegistryItem[])
        : [];
  ...
  const addFundItem = useCallback(
    (platform: RegistryItem["platform"]) => {
      const currentCfg = parseCfg(block.config);
      const currentItems = Array.isArray(currentCfg.items)
        ? (currentCfg.items as RegistryItem[])
        : [];
  ...
  const updateItem = useCallback(
    (id: string, patch: Partial<RegistryItem>) => {
      const currentCfg = parseCfg(block.config);
      const currentItems = Array.isArray(currentCfg.items)
        ? (currentCfg.items as RegistryItem[])
        : [];
  ...
  const handleOpenPopover = useCallback(
    (type: PopoverType, itemId: string, rect: DOMRect) => {
  ...
  const activeItem = popover
    ? items.find((i) => i.id === popover.itemId)
    : null;
