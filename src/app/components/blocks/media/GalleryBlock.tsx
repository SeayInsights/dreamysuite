...
export function GalleryBlock({ block }: { block: Block }) {
  ...
  const imageFit = (
    typeof cfg.imageFit === "string" ? cfg.imageFit : "cover"
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            {heading && (
              <TextEffectWrapper as="h3">{heading}</TextEffectWrapper>
            )}
            {body ? (
              <p>{body}</p>
            ) : (
              <p style={{ color: "var(--site-muted)", fontStyle: "italic" }}>
          ...
          {images.map((url, i) => (
            <div
              ...
              style={{
                position: "relative",
                width: "100%",
                paddingBottom: sized ? undefined : "75%",
