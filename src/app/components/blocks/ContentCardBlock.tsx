...
export function ContentCardBlock({ block }: { block: Block }) {
  ...
  const pushItem = useCallback((question?: string) => {
    const newItem: ContentCardItem = { id: crypto.randomUUID(), question: question || "", body: "" };
  ...
  const addLink = useCallback((itemId: string, link: LinkItem) => {
    ...
    updateBlock(block.id, {
      config: {
        ...currentCfg,
        items: currentItems.map((i) => i.id === itemId ? { ...i, links: [...(i.links ?? []), link] } : i),
      },
    });
  }, [block.id, block.config, updateBlock]);
  ...
  const deleteLink = useCallback((itemId: string, linkIndex: number) => {
    ...
    updateBlock(block.id, {
      config: {
        ...
        items: currentItems.map((i) => {
          if (i.id !== itemId) return i;
          return { ...i, links: (i.links ?? []).filter((_, idx) => idx !== linkIndex) };
        }),
      },
    });
  }, [block.id, block.config, updateBlock]);
  ...
  const handleSelect = (question: string) => { setPickerOpen(false); pushItem(question); };
  const handleCustom = () => { setPickerOpen(false); pushItem(); };
  ...
  const sectionStyle = { padding: "3rem 1.5rem", ...blockSectionStyle(cfg, breakpoint) };
  ...
  if (items.length === 0) {
    return (
      <section className="block block-content-card" data-block-id={block.id} data-block-type={block.type} style={sectionStyle}>
        {sectionHeader}
        {editing ? (
  ...
  const cardLinkHandlers = { onDelete: deleteItem, onAddLink: addLink, onEditLink: editLink, onDeleteLink: deleteLink };
      ...
      {isTravelMode && !isAccordionMode && !isListMode && cardStyle !== "numbered" && (
        <>
          <div style={cardGridStyle}>
            ...
            {editing && <div ref={addBtnRef}><AddButton onClick={openPicker} /></div>}
          </div>
          {editing && (
            <FunFactPicker
              open={pickerOpen}
