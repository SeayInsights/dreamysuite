...
export default async function PreviewPage({
  ...
  const owned = await env.DB.prepare(
    "SELECT id FROM site WHERE id = ? AND userId = ?",
  )
  ...
  if (!owned) {
    const invite = await env.DB.prepare(
      ...
      .first<{ id: string }>();
    if (!invite) redirect("/sites");
  }

  const firstPage = await env.DB.prepare(
    "SELECT id FROM page WHERE siteId = ? AND isVisible = 1 ORDER BY sortOrder ASC LIMIT 1",
  )
    .bind(siteId)
    .first<{ id: string }>();
  ...
  const rawBlocks: RawBlock[] = firstPage
    ? (
        await env.DB.prepare(
          "SELECT id, type, config, sortOrder, isVisible FROM block WHERE pageId = ? ORDER BY sortOrder ASC",
        )
          .bind(firstPage.id)
          .all<RawBlock>()
      ).results
  ...
  const universalSettings = await env.DB.prepare(
    "SELECT accentColor, bgColor, bgImage, bgImageOpacity, sectionSpacing, headingFont, bodyFont, headingColor, siteTextColor, bodyColor FROM site_setting WHERE siteId = ?",
  )
    .bind(siteId)
    .first<SiteSettings>();
