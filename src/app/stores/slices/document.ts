  ...
  reorderBlocks: (from, to) =>
    set((state) => {
      ...
      return {
        blocks,
        isDirty: true,
        pendingOps: { ...state.pendingOps, reordered: true },
      };
    }),
  resortByOffsetY: () =>
    set((state) => {
      const sorted = [...state.blocks].sort((a, b) => {
        const ay =
          typeof a.config.blockOffsetY === "number" ? a.config.blockOffsetY : 0;
        const by =
          typeof b.config.blockOffsetY === "number" ? b.config.blockOffsetY : 0;
        return ay - by;
      });
  ...
  markFlushed: (flushedOps) =>
    set((state) => {
      const remaining: PendingOps = {
        updated: new Set(
          [...state.pendingOps.updated].filter(
            (id) => !flushedOps.updated.has(id),
          ),
        ),
        inserted: new Set(
          [...state.pendingOps.inserted].filter(
            (id) => !flushedOps.inserted.has(id),
          ),
        ),
        removed: new Set(
          [...state.pendingOps.removed].filter(
            (id) => !flushedOps.removed.has(id),
          ),
        ),
        reordered: state.pendingOps.reordered && !flushedOps.reordered,
      };
      const stillDirty =
        remaining.updated.size > 0 ||
        remaining.inserted.size > 0 ||
        remaining.removed.size > 0 ||
        remaining.reordered;
