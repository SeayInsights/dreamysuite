...
export interface EditorShellSlice {
  ...
  /** Tracks which inspector accordion sections are open, keyed by section title. */
...
export const createEditorShellSlice: StateCreator<
  EditorShellSlice & ThemeSlice,
  [],
  [],
  EditorShellSlice
> = (set) => ({
  ...
  toggleSection: (title) =>
    set((s) => ({
      openSections: { ...s.openSections, [title]: !s.openSections[title] },
    })),
  setSectionOpen: (title, open) =>
    set((s) => ({
      openSections: { ...s.openSections, [title]: open },
    })),
