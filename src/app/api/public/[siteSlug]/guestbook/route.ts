...
export async function GET(
  ...
  const site = await env.DB.prepare(
    "SELECT id FROM site WHERE slug = ? AND status = 'published'",
  )
    .bind(siteSlug)
    .first<{ id: string }>();
  ...
  // Password-protected sites gate their guestbook too (mirrors the SSR route).
  const pwCookie = req.cookies.get(guestPwCookieName(siteSlug))?.value ?? null;
  if (!(await isGuestUnlocked(env.DB, site.id, pwCookie))) {
    return NextResponse.json(
      {
        error: {
  ...
  try {
    ...
    const entries = result.results.map((row) => {
      const data = safeJsonParse<{ name: string; message: string }>(row.data, {
        name: "",
        message: "",
      });
      return {
