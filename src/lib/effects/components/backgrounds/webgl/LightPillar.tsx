...
const LightPillar = ({
  topColor = "#5227FF", bottomColor = "#FF9FFC", intensity = 1.0, rotationSpeed = 0.3,
  interactive = false, glowAmount = 0.005, pillarWidth = 3.0, pillarHeight = 0.4,
  noiseIntensity = 0.5, mixBlendMode = "screen", pillarRotation = 0, quality = "high",
}: any) => {
  ...
  useEffect(() => {
    ...
    renderer.setSize(width, height); renderer.setPixelRatio(settings.pixelRatio);
    container.appendChild(renderer.domElement); rendererRef.current = renderer;
    const parseColor = (hex: string) => { const color = new THREE.Color(hex); return new THREE.Vector3(color.r, color.g, color.b); };
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        for(int i = 0; i < MAX_ITER; i++) {
          ...
          float k = 4.0; float h = max(k - abs(d - bound), 0.0); d = max(d, bound) + h * h * 0.0625 / k;
          d = abs(d) * 0.15 + 0.01;
    ...
    return () => {
      ...
      rendererRef.current = null; materialRef.current = null; sceneRef.current = null; cameraRef.current = null; geometryRef.current = null;
    };
  }, [webGLSupported, quality]);

  useEffect(() => { rotationSpeedRef.current = rotationSpeed; }, [rotationSpeed]);
  useEffect(() => { if (materialRef.current) materialRef.current.uniforms.uTopColor.value = new THREE.Vector3(...Object.values(new THREE.Color(topColor))); }, [topColor]);
  useEffect(() => { if (materialRef.current) materialRef.current.uniforms.uBottomColor.value = new THREE.Vector3(...Object.values(new THREE.Color(bottomColor))); }, [bottomColor]);
  ...
  if (!webGLSupported) return <div style={{ ...containerStyle, display: "flex", alignItems: "center", justifyContent: "center" }}>WebGL not supported</div>;
