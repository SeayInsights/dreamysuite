...
function extendMaterial(BaseMaterial: any, cfg: any) {
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]: any) => { uniforms[key] = u !== null && typeof u === "object" && "value" in u ? u : { value: u }; });
  let vert = `${cfg.header}\n${cfg.vertexHeader ?? ""}\n${baseVert}`;
  let frag = `${cfg.header}\n${cfg.fragmentHeader ?? ""}\n${baseFrag}`;
...
const MergedPlanes = forwardRef(({ material, width, count, height }: any, ref: any) => {
  ...
  useFrame((_, delta) => { (mesh.current as any).material.uniforms.time.value += 0.1 * delta; });
...
const Beams = ({ beamWidth = 2, beamHeight = 15, beamNumber = 12, lightColor = "#ffffff", speed = 2, noiseIntensity = 1.75, scale = 0.2, rotation = 0 }: any) => {
  ...
  const beamMaterial = useMemo(() =>
    extendMaterial(THREE.MeshStandardMaterial, {
      ...
      uniforms: {
        ...
        time: { shared: true, mixed: true, linked: true, value: 0 }, roughness: 0.3, metalness: 0.3,
        uSpeed: { shared: true, mixed: true, linked: true, value: speed }, envMapIntensity: 10,
