...
.dc-svg { position:relative; width:100%; height:100%; display:block; will-change:transform; }
.dc-card-text { position:absolute; bottom:1.2em; letter-spacing:-0.5px; font-weight:900; left:1em; font-size:2.5rem; line-height:1.5em; }
...
const DecayCard = ({
  ...
  return (
    <>
      <style>{decayCardStyles}</style>
      <div className="dc-content" style={{ width: `${width}px`, height: `${height}px` }} ref={svgRef}>
        <svg viewBox="-60 -75 720 900" preserveAspectRatio="xMidYMid slice" className="dc-svg">
          <filter id="imgFilter">
        ...
        <div className="dc-card-text">{children}</div>
      </div>
    </>
  );
};
