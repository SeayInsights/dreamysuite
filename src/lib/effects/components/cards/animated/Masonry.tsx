...
.masonry-item-wrapper { position:absolute; will-change:transform,width,height,opacity; padding:6px; cursor:pointer; top:0; left:0; }
...
const useMeasure = () => {
  ...
  useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver(([entry]) => { const { width, height } = entry.contentRect; setSize({ width, height }); });
...
const Masonry = ({
  ...
  const getInitialPosition = (item: GridItem) => {
    ...
    if (animateFrom === 'random') { const dirs = ['top','bottom','left','right']; direction = dirs[Math.floor(Math.random()*dirs.length)]; }
    switch (direction) {
  ...
  const grid = useMemo(() => {
    ...
    return items.map(child => {
      ...
      const x = columnWidth * col; const height = child.height / 2; const y = colHeights[col];
  ...
  useLayoutEffect(() => {
    ...
    grid.forEach((item, index) => {
      ...
      if (!hasMounted.current) {
        ...
        gsap.fromTo(selector, { opacity: 0, x: initialPos.x, y: initialPos.y, width: item.w, height: item.h, ...(blurToFocus && { filter: 'blur(10px)' }) },
  ...
  return (
    <>
      ...
      <div ref={containerRef} className="masonry-list">
        {grid.map(item => (
          <div key={item.id} data-key={item.id} className="masonry-item-wrapper"
