...
class Media {
  ...
  update(scroll: Scroll) {
    ...
    const planeHeight = this.plane.scale.y; const viewportHeight = this.viewport.height;
    const topEdge = this.plane.position.y + planeHeight / 2; const bottomEdge = this.plane.position.y - planeHeight / 2;
...
class FPCanvas {
  ...
  createPreloader() {
    ...
    this.items.forEach(src => {
      ...
      image.onload = () => { this.loaded += 1; if (this.loaded === this.items.length) { document.documentElement.classList.remove('loading'); document.documentElement.classList.add('loaded'); } };
    });
  }
  onResize() {
...
export default function FlyingPosters({
  ...
  return (
    <>
      <style>{flyingPostersStyles}</style>
